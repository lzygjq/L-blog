...
sidebar: CI/CD 与发布
desc: 从"提交到不可变制品"、发布策略与不中断、GitOps 与渐进式交付、回滚与数据库变更四条线，加上"对齐引擎 vs 放量引擎"的分层、与其他板块的分工，以及 48 题面试索引
---
...
现在拆成四篇：**先讲清从提交到制品的这条链**（CI 的节律与门禁、制品治理与供应链），**再把四种发布策略与"不中断"的真实难度讲透**，**接着把"谁在同步状态、谁在决定放量"分成两层**，**最后收口到发布里唯一不可逆的部分——数据**。
...
> 四篇内容页共 **48 题**，上面只列了最该先背的 12 条；各篇末尾都有自己的完整题单。
...
| 你的处境 | 建议顺序 |
|---|---|
| 第一次系统接触 | 导览 → 流水线设计 → 发布策略 → GitOps 与渐进式交付 → 回滚与数据库变更 |
...
| 面试准备 | 上面 12 条索引 → 各篇末尾题单（尤其发布策略篇的探针/超时题） |
