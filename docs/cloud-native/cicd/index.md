...
sidebar: CI/CD 与发布
desc: 从提交与协作、到不可变制品、发布策略与不中断、GitOps 与渐进式交付、回滚与数据库变更、研发效能度量六条线，加上对齐引擎与放量引擎的分层、与其他板块的分工，以及 72 题面试索引
---
...
现在拆成六篇：**先从提交本身说起**（分支模型、提交粒度与可评审性），**再讲清从提交到制品的这条链**（CI 的节律与门禁、制品治理与供应链），**接着把四种发布策略与"不中断"的真实难度讲透**，**然后把"谁在同步状态、谁在决定放量"分成两层**，**再收口到发布里唯一不可逆的部分——数据**，**最后站远一步看这条链的产出**（效能怎么衡量、指标怎么被误用）。
...
| [五、研发效能度量](/cloud-native/cicd/engineering-metrics) | DORA 五项 · 团队原型 · 质量成本 | **指标一旦变成目标，就不再是好指标**——度量的用途是提出改进假设，不是排名 | 五项 = 三项吞吐 + 两项不稳定（返工率 2024 新增）；2025 取消 Elite 分档改七个团队原型；**AI 采用率 +25% → 交付稳定性 −7.2%**；上游排队占前置时间的 70%~90% |
...
> 六篇内容页共 **72 题**，上面只列了最该先背的 12 条；各篇末尾都有自己的完整题单——**Git 协作篇的 12 题里，第 1~5 题值得优先看**（它们问的不是概念，而是会不会在真实操作中丢掉代码），**研发效能度量篇的 12 题则集中在「口径怎么定」与「指标怎么被玩」。**
...
| 要建效能度量 / 被要求汇报交付效能 | [研发效能度量篇 §三 口径怎么定](/cloud-native/cicd/engineering-metrics#measurement) → [§七 四条铁律与落地节奏](/cloud-native/cicd/engineering-metrics#rollout) |
