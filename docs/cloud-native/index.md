...
| 3 | [**CI/CD 与发布**](/cloud-native/cicd/) | 发布**不可重复、不可回滚** | 6 篇：Git 协作与提交粒度 / 流水线设计与制品治理 / 发布策略与不中断 / GitOps 与渐进式交付 / 回滚与数据库变更 / 研发效能度量 | ✅ 已成篇 |
| 4 | [**监控与可观测**](/cloud-native/observability/) | 出问题**看不见、定位慢** | 6 篇：指标与 PromQL / 日志管道 / 链路与 OTel / SLO 与告警 / 慢请求剧本 / 成本与许可——三支柱靠 `service` + `traceId` 关联 | ✅ 已成篇 |
...
| **在排查线上问题** | [慢请求剧本](/cloud-native/observability/slow-request) → [黄金信号](/cloud-native/observability/slo-and-alerting#signals) → [PromQL 速查](/cloud-native/observability/metrics-prometheus#promql) |
