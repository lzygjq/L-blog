...
| 板块 / 主线 | 覆盖范围 |
|---|---|
| [**成长路线**](/projects/architect-roadmap/) | **纵向主线**——四级能力阶梯（开发 → 高级开发 → 架构师 → CIO）、贯穿四级不占级别的 38 页地基（网络 / 操作系统 / 算法 / Java 语言基础 / 集合）、领域依赖链；每一级给出清单、读到哪一层就停。**AI 已挂进四级**（闸门 → 循环 → 主链路 → 试点边界），切片见[AI 应用 · 按四级读](/ai/#by-level) |
