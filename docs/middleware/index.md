...
> **本板块还有一个平级分组：[RPC 与协议](/middleware/rpc/)。** 它回答的同样是"服务之间怎么说话"，但答案是**同步调用**而不是异步事件——两者不是替代关系，而是"必须立刻知道结果"与"可以事后再处理"的分工。
...
| [**RPC 与协议**](/middleware/rpc/) | 服务间**同步调用**、**不是 MQ**（故独立成组） | 八条失效的本地调用假设、序列化与 IDL、协议与传输、Dubbo 机制（SPI / 暴露引用 / 集群容错）、超时与重试 | ✅ 5 篇已成篇（2026-09-16 新增） |
...
| **同步调用 vs 异步事件** | 什么时候必须换成 MQ、什么时候直接调下游 | [RPC 与协议](/middleware/rpc/)（[与 MQ 的分界判据](/middleware/rpc/rpc-essentials#vs-mq)） |
