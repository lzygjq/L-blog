...
| 菜单 | 定位 | 解决的问题 | 状态 |
|---|---|---|---|
| **[Spring](/java/spring/spring-framework/)** | 核心容器 + 横切能力 | 对象管理与横切逻辑：IoC、Bean、AOP、事务、ORM 集成；以及缓存、异步、重试、校验、序列化、出站 HTTP | **已成篇（13 篇）** |
| **[Spring MVC](/java/spring/spring-mvc/)** | Web 层 | 请求怎么被路由到 Controller、参数怎么绑定、响应怎么序列化；以及**接口对外该长什么样**（状态码 / 错误契约 / 分页 / 演进） | 已成篇（**2 篇**） |
| **[Spring Boot](/java/spring/spring-boot/)** | 提效 | 消除配置：自动配置、Starter、启动流程、配置体系、Actuator、内嵌容器、应用日志 | **已成篇（7 篇）** |
...
| **横切能力** | 缓存抽象、异步执行与线程池、重试与并发限制、参数校验与统一异常、序列化与类型边界、出站 HTTP 超时 |

详见 [Spring · 导览](/java/spring/spring-framework/)。

**前四块与第五块的关系**：前四块讲**机制**（容器的组织、Bean 的生死、代理的织入、SQL 的执行），第五块讲**这四块机制在真实项目里的落地形态**——`@Cacheable`／`@Async`／`@Retryable`／`@Valid`／`@JsonXxx` 这五类注解，**每一个都建立在代理与生命周期之上，而失效时都不报错**。如果时间紧，**前四块是地基，必须先读**；第五块可以在遇到具体问题时按需查。
