...
//   - SpeechReader.vue：语音朗读（由 RightRail 引用）—— 把本页正文读出来，做法是
//     浏览器原生 Web Speech API：零依赖 / 零后端、无需 API key、文本不上传。
//     段间排队（防单条超长被静默截断）、跳过代码块与表格、从当前屏幕位置起读
