...
desc: 从「能改功能」到「能独立交付」的七步阅读顺序，每一步给出页清单与「读到哪一层就停」，另附本级跳过的内容、离场自检六个问题与本站暂无的部分
order: 1
---
...
以下三块**属于 L1 该有的内容，但本站目前没有专篇**（缺口依据见[成长路线的覆盖度一节](/projects/architect-roadmap/#coverage)）。写在这里是为了避免你以为是漏读了：

| 缺口 | 建议替代 |
|---|---|
| **Git 与团队协作**（分支模型 / 代码评审 / 提交规范 / 冲突处理） | 《Pro Git》前五章 + 团队现有规范 |
| **依赖管理与构建**（Maven 依赖冲突 / 多模块 / 构建加速） | Maven 官方文档 + `mvn dependency:tree` 实践 |
| **REST 接口设计**（状态码 / 分页 / 版本 / 错误契约） | 先看[参数校验与统一异常](/java/spring/spring-framework/crosscutting/validation)建立错误响应的概念，其余查 RFC |
