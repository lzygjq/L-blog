...
|---|---|---|
| **L1 开发** | 语言与集合 → 设计原则与常用模式 → 数据「会用」→ 框架「会用」→ 测试 → 排查入门 | 从「能改」到「能独立交付」：补上测试与排查这两块以前从没被要求过的东西 |
| **L2 高级开发** | 并发 → JVM → 数据原理 → 框架原理 → 消息 → 第二数据库 | 从「交付」到「负责」：能把线上问题定位到根因，而不是「重启一下试试」 |
| **L3 架构师** | 分布式地基 → 通信 → 数据架构 → 方法论 → 非功能 → 工程效能 | 从「负责」到「设计」：给出方案，并为方案的代价负责 |
| **L4 CIO** | 技术战略 → 算账 → 风险与合规 → 落地叙事 → 业务语言 | 从「设计」到「规划」：用业务语言表述技术投入与回报 |
...
这一层不是「学」出来的，是前四层积累后的自然跃迁。面试架构师岗时，考的就是能否站在这个高度做决策。**本站对应 L4，三项缺口已于 2026-09-17 补齐**（研发效能度量 / 技术成本与 ROI / 合规体系），仍不覆盖的部分见下一节。
...
按级别盘一遍全站检索的结果。**缺的写出来，比让读者自己撞墙好**：

| 级别 | 缺口现状（2026-09-17 更新） |
|---|---|
| **L1 开发** | ✅ **已补齐**：[Git 协作](/cloud-native/cicd/git-collaboration)、[依赖管理与构建](/java/basics/dependency-build)、[REST 接口设计](/java/spring/spring-mvc/rest-api-design) |
| **L2 高级开发** | ✅ **已补齐**：[连接池](/database/mysql/connection-pool)（此前正文有 48 处提到它，却没有专篇） |
| **L3 架构师** | 暂无成规模的缺口 |
| **L4 CIO** | ✅ **已补齐**：[研发效能度量](/cloud-native/cicd/engineering-metrics)、[技术成本与 ROI](/methodology/tech-cost-roi)、[合规体系](/security/compliance-system) |

上表里 **L1 的三项正是「开发第一天就要用」的东西**，已于 2026-09-17 全部补齐——它们不属于技术深度，而属于「**代价由别人承担、所以最容易被自己忽略**」的那一类：分支模型决定你的提交能不能被评审，依赖管理决定构建能不能复现，接口契约决定调用方要不要为你的接口写特殊逻辑。**L4 的三项已于同日补齐**——它们的难点不在技术深度，而在**把组织、预算与风险里的判断翻译成可验证的机制**；补齐之后 L4 仍有本站不覆盖的部分（组织设计、财务与法务判定），那些已在该级页明确列为「知识域边界」而不是内容缺口。

**每一项缺口在对应级别页里都有落点**——L1、L2、L4 的该节均为「**已补齐**」并标明本级该读到哪一层（见 [L1 开发](/projects/architect-roadmap/developer/)、[L2 高级开发](/projects/architect-roadmap/senior/)、[L4 CIO](/projects/architect-roadmap/cio/)）；L3 的该节解释的是「**为什么这一级缺的不是内容**」，所以它没有「已补齐」这一步，见 [L3 架构师](/projects/architect-roadmap/architect/)。这里是清单，那里是可操作的建议。
...
| 按深度走路线 | 本页 → 四级清单：[L1 开发](/projects/architect-roadmap/developer/) → [L2 高级开发](/projects/architect-roadmap/senior/) → [L3 架构师](/projects/architect-roadmap/architect/) → [L4 CIO](/projects/architect-roadmap/cio/) |
