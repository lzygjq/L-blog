...
| **AI 进主链路的取舍** | LLM 失败时业务怎么办、要不要编排、试点怎么设边界——[Spring AI](/ai/spring-ai/)（L3）与[五道闸门](/ai/vibe-coding/#five-gates)（L4）；机制收口见[AI 治理](/ai/governance/) |

这一层不是「学」出来的，是前四层积累后的自然跃迁。面试架构师岗时，考的就是能否站在这个高度做决策。**本站对应 L4，效能 / 成本 / 合规 / AI 治理已成篇**，仍不覆盖的部分见下一节。
...
按级别盘一遍全站检索的结果。**缺的写出来，比让读者自己撞墙好。** 百科型缺口（Git / 依赖 / REST、连接池、ROI / 效能 / 合规）已于 2026-09-17 补齐；交付闭环三批已落地（接清单 → 应用日志 / 出站超时 / 慢请求 / 复盘 → 方案评审 / 对账 / 多租户 / AI 治理）。**现在缺的仍不是「还少哪一章中间件」**：
