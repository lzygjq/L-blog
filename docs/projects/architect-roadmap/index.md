...
| 级别 | 主要缺口 |
|---|---|
| **L1 开发** | **Git 与团队协作**（分支模型 / 代码评审 / 提交规范）、**依赖管理与构建**（Maven 依赖冲突 / 多模块 / 构建加速）、**REST 接口设计**（状态码 / 分页 / 版本 / 错误契约） |
| **L2 高级开发** | **连接池**（HikariCP 参数 / 连接泄漏 / 池大小推导）——正文里 48 处提到它，但没有专篇 |
| **L3 架构师** | 暂无成规模的缺口 |
| **L4 CIO** | **研发效能度量**（DORA 四项 / 团队拓扑）、**技术成本与 ROI 模型**、**合规体系**（等保 / 数据出境 / 开源许可治理） |

上表里 **L1 的三项正是「开发第一天就要用」的东西**，目前建议先看官方文档或《Pro Git》。L4 的三项缺口也解释了为什么这一级显得单薄——它的主战场大多不在「技术知识」里，而在组织、预算与风险这些本站基本不覆盖的区域。

**每一项缺口的替代建议，写在对应级别页的「本站暂无的内容」一节**（L2 的连接池缺口见 [L2 高级开发](/projects/architect-roadmap/senior/)，L4 的三项见 [L4 CIO](/projects/architect-roadmap/cio/)）。这里是清单，那里是可操作的建议。
