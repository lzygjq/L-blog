...
desc: 从「交付」到「负责」的七步阅读顺序——并发、JVM、数据原理、框架原理、消息、第二数据库与工程质量，每一步给出页清单与「读到哪一层就停」，另附本级跳过的内容、离场自检六个问题与本站暂无的部分
order: 2
---
...
| 缺口 | 建议替代 |
|---|---|
| **连接池**（HikariCP 参数 / 连接泄漏 / 池大小推导） | 正文里有 48 处提到连接池但没有专篇。先看[容量测算与压测方案](/projects/property-saas/capacity-and-perf/#sizing)理解「池大小不是越大越好」，再对照 HikariCP 官方文档的 `maximumPoolSize` 与 `connectionTimeout`，并自己用 `SHOW PROCESSLIST` 验证一次泄漏 |
